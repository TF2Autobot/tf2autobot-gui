...
export async function get(start: number, interval: number, end: number, enableTrades?: boolean): Promise<Profit> {
	...
	for (let i = 0; i < trades.length; i++) { // TODO: ADMIN TRADES
		...
		for (const sku in trade.dict.their) { // items bought
...
class profitTracker {
	...
	constructor(start: number, interval: number, end: number, currentKey: number) {
...
class itemTracker {
	...
	private overItems: Items;

	private itemPricePlot: Items;

	profitTrack: profitTracker;
	 ...
	 */
	boughtItem(itemCount: number, sku: string, prices: Currency, rate: number, time: number) {
		let itemProfit = 0;
	 ...
	 * @return {Number} profit made on this item
	 */
	soldItem(itemCount: number, sku: string, prices: Currency, rate: number, time: number): number {
		let itemProfit = 0;
