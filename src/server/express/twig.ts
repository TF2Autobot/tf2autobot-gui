...
export = function init(app: Express): void {
    Twig.extendFunction('load_entry_tags', function (entrypoint, type){
        let entrypoints = require(path.join(require(path.join(process.cwd(), 'webpack.config.js')).output.path, './entrypoints.json')).entrypoints // TODO: cache so we are not loading files with every access
